#!/usr/bin/env python3
"""
Score generated notes using Gemini 2.5 Pro as judge.

Same rubric and scoring methodology as score-notes.py (Claude Opus judge),
but uses Gemini for a second-opinion comparison.

Usage:
    python evals/scripts/score-notes-gemini.py                          # score all unscored notes
    python evals/scripts/score-notes-gemini.py --template soap           # one template only
    python evals/scripts/score-notes-gemini.py --model medgemma-1.5-4b   # one model only
    python evals/scripts/score-notes-gemini.py --transcript 01            # one transcript only
    python evals/scripts/score-notes-gemini.py --dry-run                  # show what would be scored
    python evals/scripts/score-notes-gemini.py --max 10                   # score at most 10 notes
"""
...
SCORES_DIR = EVALS_DIR / "results" / "gemini-pro-judge"
...
# ---------------------------------------------------------------------------
# Scoring rubric (identical to score-notes.py for consistency)
# ---------------------------------------------------------------------------
...
def get_gemini_client():
    """Create Google GenAI client, loading API key from env or .env file."""
        ...
        if env_file.exists():
            for line in env_file.read_text().splitlines():
                if line.startswith("GOOGLE_API_KEY="):
                    ...
                    break
    if not api_key:
        print("ERROR: GOOGLE_API_KEY not set. Add it to .env or environment.",
              file=sys.stderr)
        sys.exit(1)
...
def score_note(client, transcript_text: str, template_text: str, note_text: str) -> dict:
    """Send a note to Gemini Pro for scoring. Returns parsed score dict."""
    from google import genai
    ...
    full_content = f"{JUDGE_SYSTEM_PROMPT}\n\n{user_prompt}"

    start = time.perf_counter()
    response = client.models.generate_content(
        model=JUDGE_MODEL,
        contents=[
            {"role": "user", "parts": [{"text": full_content}]},
        ],
        config={
            "temperature": 0.0,
...
def main():
    parser = argparse.ArgumentParser(description="Score generated notes with Gemini Pro")
    ...
    for template_name, transcript_id, model_key, response_file, already_scored in runs:
        ...
        try:
            ...
            if "error" in scores:
                print(f"PARSE ERROR: {scores['error']}")
                errors += 1
            else:
